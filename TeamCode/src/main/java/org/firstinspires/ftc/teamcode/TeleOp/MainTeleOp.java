...
public class MainTeleOp extends OpMode {
    ...
    public void loop(){
        ...
        if((Math.abs(Robot.drive.localizer.getPose().position.x) > 50 || Math.abs(Robot.drive.localizer.getPose().position.y) > 50)
                && Math.abs(gamepad1.right_stick_x) > 0.0){
            ...
            double snapWeight = Math.max(0, 15*(Math.abs(Math.cos(headingError))-Math.cos(Math.toRadians(20))));
